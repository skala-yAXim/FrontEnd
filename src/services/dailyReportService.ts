...
export class DailyReportService {
   ...
   * Daily 보고서 생성
   */
  static async createReport(data: CreateDailyReportRequest): Promise<DailyReport> {
    try {
   ...
   * Daily 보고서 수정
   */
  static async updateReport(data: UpdateDailyReportRequest): Promise<DailyReport> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/daily-reports/${data.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          // TODO: Authorization 헤더 추가
