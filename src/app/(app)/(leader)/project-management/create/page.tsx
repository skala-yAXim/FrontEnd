...
export default function ProjectCreatePage() {
  ...
  // 폼 값 변경 핸들러
  const handleFormChange = (field: keyof Omit<ProjectCreateForm, 'files'>, value: string) => {
    setForm(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Array.from(files).forEach(file => {
      ...
      if (!isValidType) {
        newErrors.push(`${file.name}: 허용되지 않는 파일 형식입니다. (docx, xlsx, xls, txt만 허용)`);
        return;
      }
      ...
      if (form.files.some(f => f.name === file.name)) {
        newErrors.push(`${file.name}: 동일한 이름의 파일이 이미 첨부되어 있습니다.`);
        return;
      }
        ...
        <CardContent className="space-y-6">
          ...
          {fileToDelete && fileToDeleteInfo && (
            <Alert variant="destructive">
              <AlertDescription>
                <div className="space-y-3">
                  <p>
                    <strong>"{fileToDeleteInfo.name}"</strong> 파일을 삭제하시겠습니까?
                  </p>
                  <div className="flex gap-2">
                    <Button size="sm" variant="destructive" onClick={handleFileDeleteConfirm}>
                      삭제
                    </Button>
                    <Button size="sm" variant="outline" onClick={handleFileDeleteCancel}>
                      취소
                    </Button>
                  </div>
                </div>
          ...
          <div className="space-y-4">
            ...
            {errors.files && (
              <Alert variant="destructive">
                ...
                <AlertDescription>
                  <pre className="whitespace-pre-wrap text-sm">{errors.files}</pre>
                </AlertDescription>
              </Alert>
            )}
            ...
            {form.files.length > 0 && (
              <div className="space-y-2">
                <p className="text-sm font-medium">첨부된 파일 ({form.files.length}/{MAX_FILE_COUNT})</p>
                <div className="space-y-2">
                  {form.files.map(file => (
                    <div
                      key={file.id}
          ...
          <div className="flex justify-end gap-3 pt-4">
            <Button
              ...
              disabled={showConfirmCreate || showConfirmCancel || fileToDelete !== null}
            >
              취소
            </Button>
            <Button
