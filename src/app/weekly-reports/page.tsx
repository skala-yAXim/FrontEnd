...
const generateDummyReports = (): WeeklyReportItem[] => {
  ...
  const statuses: ('completed' | 'generating' | 'draft' | 'error')[] = ['completed', 'generating', 'draft', 'error'];
...
export default function WeeklyReportsPage() {
  ...
  const handleCreateNew = () => {
    router.push('/weekly-reports/create');
  };
              ...
              {paginatedReports.length === 0 ? (
                ...
                paginatedReports.map((report) => (
                  <tr 
                    ...
                    title={report.status === 'completed' ? '클릭하여 상세보기' : '완료된 보고서만 볼 수 있습니다'}
                  >
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6 pt-4 border-t">
            ...
            <div className="text-sm text-muted-foreground">
              ...
              <strong>{Math.min(currentPage * itemsPerPage, totalItems)}</strong>개 표시
            </div>
