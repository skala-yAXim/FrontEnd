...
export const useWeeklyReportStore = create<WeeklyReportState>((set, get) => ({
  ...
  // 보고서 생성
  createReport: async (
    data: WeeklyReportCreateRequest
  ): Promise<WeeklyReport> => {
  ...
  updateReport: async (
    ...
    try {
      ...
      set({
        ...
        currentReport:
          currentReport?.id === id ? response.report : currentReport,
        isSubmitting: false,
      });
...
export const selectCurrentReport = (state: WeeklyReportState) =>
  state.currentReport;
...
export const selectIsSubmitting = (state: WeeklyReportState) =>
  state.isSubmitting;
